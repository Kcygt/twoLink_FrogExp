...
%  Parameters
time = [10 20];      % time
wn = 2;              % Prefilter Omega     
kj = [40 25];        % Spring  [q1 q2]
bj = [10 30];        % Damping [q1 q2]
wt = [500, 5, 100];  % weights [qDes, Time, qMid]
...
lb = [0 0   1.5   10  10    2   2     ];   % Lower bounds
ub = [2 6   50    200 200   200 200 ];     % Upper bounds
...
[t, y] = ode45(@(t, x) myTwolinkwithprefilter(t, x, optimalParams(3), optimalParams(1:2), qDes, optimalParams(4:5), optimalParams(6:7)), [0 optimalParams(2)], zeros(8, 1));
...
function error = objectiveFunction(params, qDes,wt,qMid)
    ...
    % Simulate the system
    [t, y] = ode45(@(t, x) myTwolinkwithprefilter(t, x, params(3), params(1:2), qDes, params(4:5), params(6:7)), [0 params(2)], x0);
