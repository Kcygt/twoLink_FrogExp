...
%  Parameters
time = [10 20];      % time
wn = [2 2];              % Prefilter Omega     
kj = [40 25];        % Spring  [q1 q2]
bj = [10 30];        % Damping [q1 q2]
wt = [500, 100, 100];  % weights [qDes, Time, qMid]

% Optimization setup
initParams = [time  wn bj kj]; % Initial guess for [time, wn, bj, kj]

[init_T, init_Y] = ode45(@(t, x) myTwolinkwithprefilter(t, x, wn, time, qDes, bj, kj),   [0 time(end)], zeros(8, 1));

% Lower and upper boundaries 
lb = [0 0   2 2   10  10    2   2     ];   % Lower bounds
ub = [2 6   2 2 200 200   200 200 ];     % Upper bounds
...
function error = objectiveFunction(params, qDes,wt,qMid)
    ...
    % Simulate the system
    [t, y] = ode45(@(t, x) myTwolinkwithprefilter(t, x, params(3:4), params(1:2), qDes, params(4:5), params(6:7)), [0 params(2)], x0);
