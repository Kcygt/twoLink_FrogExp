...
% weights
wt = [1, .00001, 2000];  %  [qDes, Time, qMid]

% Optimization setup
initPrms = [ttime,zeta1,zeta2,zeta3,zeta4,zeta5,wn1,wn2,wn3,wn4,wn5];

% Initial Condition
[tinit, yinit] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, initPrms(1:5), qDes, initPrms(6:8),initPrms(9:11),initPrms(12:14),initPrms(15:17),initPrms(18:20), ...   
...
function dxdt= myTwolinkwithprefilter(t, x, t_st, qDes, zeta1,zeta2,zeta3,zeta4,zeta5,wn1,wn2,wn3,wn4,wn5)
    ...
    elseif t_st(2) <= t && t < t_st(3)
        dxdt = [A{3}*x(1:6) + B{3}*qDes(:); qd; qdd];
    elseif t_st(3) <= t && t < t_st(4)
        dxdt = [A{4}*x(1:6) + B{4}*qDes(:); qd; qdd];
    else
        dxdt = [A{5}*x(1:6) + B{5}*qDes(:); qd; qdd];
    end
