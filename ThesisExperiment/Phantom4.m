...
function error = objectiveFunction(prms, qDes, wt)
    ...
    % Simulate the system
    [tt, yy] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, prms(1:2), qDes,   prms(3:5),   prms(6:8), ...
    ...
    tMid = linspace(0,max(tt),6);
    yyintp=interp1(tt,yy,tMid(2:end-1));
    ...
    error = wt(1) * V + ...
            wt(2) * (distErr1 + distErr2 + distErr3 +distErr4) ;
