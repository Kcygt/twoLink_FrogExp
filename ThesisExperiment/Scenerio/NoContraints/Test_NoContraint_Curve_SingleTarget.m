...
kj = [90 90 90];  % Spring constants
bj = [25 25 25];  % Damping constants
zeta = [0.9 1.01 1];

wt = [0, 0, 5];  % Weights [qDes, Time, qMid]
...
lb = [2   1  1  1   35 35 35   70 70 70     0.5 0.5 0.5];   
ub = [5   3 3 3     50 50 50   100 100 100  1.1 1.1 1.1];  

% Objective Function
objectiveFunc = @(params) objectiveFunction(params, qDes, wt, xMid);

% Run optimization
options = optimset('Display', 'iter', 'TolFun', 1e-10, 'MaxIter', 4000,'TolX',1e-10,'MaxFunEvals', 1e5);
...
function error = objectiveFunction(params, qDes, wt, xMid)
    ...
    distEnd = min(sum((xAct - xDes ).^2, 2) + sum((params(1) - t).^2, 2)); 

    % distMid = sum(arrayfun(@(i) min(sum((xAct - xMid(i, :)).^2, 2)), 1:size(xMid,1)));
    distMid = min(sum(  (xAct(10:end-20,:) - xMid(1, :)).^2  , 2));

    error = wt(1) * distEnd + wt(2) * params(1) + wt(3) * distMid;
end
