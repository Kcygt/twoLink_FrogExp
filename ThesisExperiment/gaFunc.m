...
wt = [1, 0, 200];  % Weights [qDes, Time, qMid]
...
lb = [10   1  1  1    10 10 10    20  20  20    0.5  0.6  0.6 ];   
ub = [10   10 10 10   40 40 40    100 100 100   5    5    5  ];  

% Objective Function
objectiveFunc = @(params) objectiveFunction(params, qDes, wt, qMid);

% Genetik algoritma (GA) için seçenekler
options = optimoptions('ga', 'PlotFcn', 'gaplotbestf', 'MaxGenerations', 1000, 'PopulationSize', 10);

% Optimize with Genetik Algoritma (GA)
optimalParams = ga(objectiveFunc, length(initParams), [], [], [], [], lb, ub, [], options);
