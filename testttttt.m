...
t = linspace(0, 16, length(qDes)); % Time vector
...
xDesF = forward_kinematics(qDesF(:, 1), qDesF(:, 2), l1, l2);
% figure(1);
% plot(xDes(:,1),xDes(:,2))
% figure(2);
% plot(xDesF(2:end,1),xDesF(2:end,2))
% 
% figure(3);
% plot(qDes(:,1),qDes(:,2))
% figure(4);
% plot(qDesF(2:end,1),qDesF(2:end,2))
%%
% Controller gains
K = 70; % Proportional gain
...
plot(xAct(2:end, 1), xAct(2:end, 2), '-', 'DisplayName', 'End Effector Actual');
